...
if ( ! class_exists( 'avia_sc_contact' ) )
{
	class avia_sc_contact extends aviaShortcodeTemplate
	{
		...
		protected function extra_assets()
		{
			...
			//load css
			wp_enqueue_style( 'avia-module-contact', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/contact/contact{$min_css}.css", array( 'avia-layout' ), $ver );

				//load js
			wp_enqueue_script( 'avia-module-contact', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/contact/contact{$min_js}.js", array( 'avia-shortcodes' ), $ver, true );
		}
		...
		protected function register_dynamic_templates()
		{
			...
			$desc_cache = __( 'If you are using a cache plugin we recommend that you exclude the page containing the contact form from caching.', 'avia_framework' );
			...
			$c = array(
						...
						array(
							'name'		=> __( 'Cache Information', 'avia_framework' ),
							'desc'		=> $desc_cache,
							'type'		=> 'heading'
						),
						...
						array(
							'name'		=> __( 'Submit Action', 'avia_framework' ),
							'desc'		=> __( 'What should happen once the form gets sent', 'avia_framework' ),
							'id'		=> 'on_send',
							'type'		=> 'select',
							'std'		=> '',
							'lockable'	=> true,
						...
						array(
							'name'		=> __( 'Display Fields With Error', 'avia_framework' ),
							'desc'		=> __( 'On error, show a message box containing the labels of the invalid fields', 'avia_framework' ),
							'id'		=> 'with_errors',
							'type'		=> 'select',
							'std'		=> '',
							'lockable'	=> true,
			...
			$c = array(
						array(
							'name'		=> __( 'Your E-Mail Address', 'avia_framework' ),
							'desc'		=> __( 'Enter one or more E-Mail addresses (separated by comma) where mails should be delivered to.', 'avia_framework' ) . ' (' . __( 'Default:', 'avia_framework' ) . ' '. get_option( 'admin_email' ) . ')',
							'id'		=> 'email',
							'type'		=> 'input',
						...
						array(
							...
							'desc'		=> __( 'You can define a custom E-Mail Subject for your form here. If left empty the subject will be', 'avia_framework' ) . ': <small>' . __( 'New Message', 'avia_framework' ) . ' (' . __( 'sent by contact form at', 'avia_framework' ) . ' ' . get_option( 'blogname' ) . ')</small>',
							'id'		=> 'subject',
							'type'		=> 'input',
							'std'		=> '',
							'lockable'	=> true
						...
						array(
							'name'		=> __( 'Autoresponder From E-Mail Address', 'avia_framework' ),
							'desc'		=> __( 'Enter the from E-Mail address for the autoresponder.', 'avia_framework' ) . ' (' .__( 'Default:', 'avia_framework' ) . ' ' . get_option( 'admin_email' ) . ')',
						...
						array(
							'name'		=> __( 'Autoresponder Reply To E-Mail Address', 'avia_framework' ),
							'desc'		=> __( 'Enter one or more reply to E-Mail addresses (separated by comma) for the autoresponder mail. Leave blank if not needed.', 'avia_framework' ),
						...
						array(
							...
							'desc'		=> __( 'Enter a custom subject text to the autoresponder E-Mail. Leave blank to use a default message.', 'avia_framework' ),
			...
			$c = array(
						...
						array(
							...
							'subtype'	=> array(
												...
												__( 'Valid Positive Number including 0', 'avia_framework' )				=> 'is_positiv_number'
											)
						),
		...
		public function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			if( $with_errors == 'yes' )
			{
				$meta['el_class'] .= ' av-show-form-errors ';
			}
