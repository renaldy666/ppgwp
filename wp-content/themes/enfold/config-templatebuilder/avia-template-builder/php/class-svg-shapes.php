...
if( ! class_exists( 'aviaSvgShapes' ) )
{
	...
	class aviaSvgShapes
	{
		...
		/**
		 * Returns the filtered subdirectories below WP uploads directory that holds the custom svg shape files.
		 * Any other custom directory can be used via filter 'avf_custom_svg_shapes_files_directory'
		 *
		 * @since 4.8.4
		...
		public function custom_files_directory()
		{
			...
			if( is_null( $this->custom_files_dir ) )
			{
				...
				$dir = trailingslashit( $wp_upload_dir['basedir'] . trailingslashit( $avia_config['dynamic_files_upload_folder'] ) . 'avia_custom_shapes' );
		...
		/**
		 * Return cached svg file content
		 *
		 * @since 4.8.4
		 * @param array $params
		 * @return string
