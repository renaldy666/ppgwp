 ...
 * Can be used in addition to class avia_slideshow. Kept to allow backwards compatibility.
 *
 * Use action 'ava_builder_core_files_loaded' or 'ava_builder_shortcode_files_loaded' to load if needed.
 *
 * @since ???
 * @since 4.8.9				class renamed to avia_slideshow_old
 * @deprecated since 4.8.9
 */
if( ! defined( 'ABSPATH' ) ) {  exit;  }    // Exit if accessed directly


if( ! class_exists( 'avia_slideshow_old' ) )
{
	/**
	 * @deprecated since 4.8.9
	 */
	class avia_slideshow_old
	{
		...
		public function __construct( array $config )
		{
			...
			$this->config = array_merge( avia_slideshow_old::default_args(), $config );
			 ...
			 * @return string
			 */
			$this->config['lightbox_size'] = apply_filters( 'avf_alb_lightbox_image_size', $this->config['lightbox_size'], 'avia_slideshow_old', $this->config, '' );
		...
		public function html()
		{
			...
			$html .= "<div {$this->config['el_id']} {$data} class='avia-slideshow avia-slideshow-" . avia_slideshow_old::$slider . " {$extraClass} avia-slideshow-{$this->config['size']} {$this->config['handle']} {$this->config['class']} avia-{$this->config['animation']}-slider ' {$markup}>";
		...
		protected function advanced_slide()
		{
			...
			foreach( $this->id_array as $key => $id )
			{
				...
				if( isset( $this->slides[ $id ] ) || $slide_type == 'video' )
				{
					...
					if( $slide_type == 'video' )
					{
						$this->service    = avia_slideshow_video_helper_old::which_video_service( $video );
						$video 			  = avia_slideshow_video_helper_old::set_video_slide( $video, $this->service, $meta, $this->config, $markup_video );
					...
					$html .=	"<{$tags[0]} data-rel='slideshow-" . avia_slideshow_old::$slider . "' class='avia-slide-wrap {$fallback_img_class}' {$fallback_img_style} {$linkdescription} {$markup_video}>{$caption}";
		...
		protected function extract_subslides( array $slide_array )
		{
			...
			foreach( $slide_array as $key => $slide )
			{
				...
				if( empty( $slide['attr']['id'] ) && ! empty( $slide['attr']['video'] ) && $slide['attr']['slide_type'] === 'video' )
				{
					...
					if( avia_slideshow_video_helper_old::is_extern_service( $slide['attr']['video'] ) )
					{
...
if( ! class_exists( 'avia_slideshow_video_helper_old' ) )
{
	/**
	 * @deprecated since 4.8.9
	 */
	class avia_slideshow_video_helper_old
	{
		...
		{
			$ervice = avia_slideshow_video_helper_old::which_video_service( $video_url );

			return in_array( $ervice, avia_slideshow_video_helper_old::$extern_services );
		}
	}
}
